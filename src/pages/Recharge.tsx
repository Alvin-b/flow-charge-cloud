  ...
  const handlePay = async () => {
    ...
    try {
      ...
      const pollTimer = setInterval(async () => {
        if (payStateRef.current !== "pending") { clearInterval(pollTimer); return; }
        attempts++;
        try {
          const queryResult = await fetchWithTimeout(() => mpesaApi.querySTKStatus(checkoutId), 12000);
          if (payStateRef.current !== "pending") return;
          if (queryResult.status === "completed") {
            cleanup(); Sounds.success(); updatePayState("success");
            const { data } = await supabase.from("wallets").select("balance_kwh").maybeSingle();
            if (data) setWalletBalance(data.balance_kwh);
          } else if (queryResult.status === "failed" || queryResult.status === "cancelled" || queryResult.status === "timeout") {
            cleanup(); Sounds.error(); updatePayState("failed");
            toast({ title: "Payment not completed", description: queryResult.result_desc, variant: "destructive" });
          }
        } catch (err) { console.warn("STK query poll error:", err); }
  ...
  if (payState === "success") {
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center px-6 gap-6">
        ...
        <div className="w-full max-w-sm glass-card-elevated rounded-2xl p-5 space-y-3 animate-fade-in-up" style={{ animationDelay: "0.2s" }}>
        ...
        <Button onClick={() => { setPayState("idle"); setAmount(""); navigate("/"); }} className="w-full max-w-sm h-13 bg-primary text-primary-foreground font-bold rounded-xl">
          Done
        </Button>
      </div>
    );
  ...
  if (payState === "failed") {
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center px-6 gap-6">
        ...
        <Button onClick={() => setPayState("idle")} className="bg-primary text-primary-foreground font-bold rounded-xl px-8 h-12">Try Again</Button>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-background pb-24">
      ...
      <div className="px-5 space-y-5">
        <div className="glass-card-elevated rounded-2xl p-4 flex items-center justify-between animate-fade-in-up">
          <div>
        ...
        <div className="animate-fade-in-up" style={{ animationDelay: "0.1s" }}>
          ...
          <div className="flex flex-wrap gap-2">
            {PRESETS.map((p) => (
              <button key={p} onClick={() => { Sounds.tap(); setAmount(p.toString()); }} className={cn(
                "px-4 py-2.5 rounded-xl text-sm font-semibold border transition-all active:scale-95",
                amount === p.toString()
        ...
        <div className="animate-fade-in-up" style={{ animationDelay: "0.15s" }}>
          ...
          <div className="relative">
            ...
            <input type="number" placeholder="Enter amount" value={amount} onChange={(e) => setAmount(e.target.value)}
              className="w-full pl-14 pr-4 py-4 bg-card rounded-xl border border-border text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary text-lg font-bold transition-all" />
          </div>
          {amount && (
            <div className="mt-3 glass-card-elevated rounded-xl p-3 flex items-center justify-between animate-scale-in">
        ...
        <div className="animate-fade-in-up" style={{ animationDelay: "0.2s" }}>
          ...
          <div className="relative">
            ...
            <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)}
              className="w-full pl-11 pr-4 py-4 bg-card rounded-xl border border-border text-foreground focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary font-medium transition-all" />
          </div>
        </div>

        <div className="glass-card-elevated rounded-xl p-3 animate-fade-in-up" style={{ animationDelay: "0.25s" }}>
        ...
        <Button onClick={handlePay} disabled={!amount || parseFloat(amount) < 10 || !phone.trim()}
          className="w-full h-14 bg-primary text-primary-foreground font-bold text-base rounded-xl hover:bg-primary/90 transition-all disabled:opacity-40 animate-fade-in-up" style={{ animationDelay: "0.3s" }}>
