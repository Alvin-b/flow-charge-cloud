...
import {
  ...
  Sparkles, Clock, Shield, Cpu, Radio
} from "lucide-react";
    ...
    <div className="min-h-screen pb-28 bg-background cyber-grid noise-overlay relative">
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1.5 bg-secondary/50 border border-border rounded-lg px-3 py-1.5">
            ...
            <span className="text-[10px] font-mono text-muted-foreground">
              {meterCount > 0 ? `${meterCount} ONLINE` : "OFFLINE"}
            </span>
          </div>
          <button onClick={() => { Sounds.tap(); navigate("/notifications"); }}
            ...
            {unreadCount > 0 && (
              <motion.span
                initial={{ scale: 0 }}
                animate={{ scale: 1 }}
                className="absolute top-1.5 right-1.5 w-2.5 h-2.5 rounded-full bg-primary glow-cyan"
              />
            )}
          </button>
        </div>
      ...
      >
        ...
        <motion.div variants={stagger.item}
          className="rounded-2xl p-6 gradient-wallet overflow-hidden relative hud-corners"
        >
        ...
        <motion.div variants={stagger.item}>
          {meterCount > 0 ? (
            <motion.div
              whileTap={{ scale: 0.98 }}
              className="glass-card-elevated rounded-2xl p-4 cursor-pointer neon-border"
            ...
            >
              ...
              <p className="text-xs text-muted-foreground font-mono mt-1.5 max-w-[220px] mx-auto leading-relaxed">
                Scan QR or enter meter code
              </p>
