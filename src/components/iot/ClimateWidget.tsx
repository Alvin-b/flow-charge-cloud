            ...
            <div className="flex-1 min-w-0">
              ...
              {device.type === "thermostat" && device.on && (
                <div className="flex items-center gap-2 mt-1">
                  <span className="text-[10px] text-muted-foreground">Now: {device.temp}°C</span>
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-xs font-bold text-success w-8 text-center">{device.target}°C</span>
                    <button onClick={() => adjustTemp(device.id, 1)} className="w-5 h-5 rounded bg-muted/20 flex items-center justify-center hover:bg-muted/40">
              ...
              {device.type === "humidity" && device.on && (
                <span className="text-[10px] text-muted-foreground">Humidity: {device.humidity}%</span>
              )}
            </div>
          </div>
        ))}
